...
const ViewComplaints = () => {
    ...
    const handleAssignLawyer = (incident) => {
        ...
        const transferComplaint = async (firId, lawyerAddress) => {
            const contract = await getNFT();
            const tx = await contract.assignToLawyer(firId, lawyerAddress);
            return await tx.wait(1);
        };
        const address = lawyerAddress;
        const firId = incident.complaintId;
        console.log(address, firId);
        transferComplaint(firId, address).then(
            (id) => {
                setProgressMessage("Transaction in progress...");
                console.log("Complained assigned to lawyer", id);
                instance
            ...
            (err) => {
                ...
                if (err.info.error.code == 4001) {
                    setErrorMessage("User denied the ETH Transaction");
                } else {
                    setErrorMessage("Internal Server Error. Retry again!");
                }
                incident.status = "Open";
                incident.lawyerAssigned = "";
                instance.put(`update-fir`, incident).then((response) => {
                    console.log(response);
                    openAlert(true);
                    setTimeout(() => {
                        setIsDialogLoading(false);
    ...
    const handleCloseIncident = (incident) => {
        ...
        const caseSolved = async ({ complaintId }) => {
            const contract = await getNFT();
            const tx = await contract.caseSolved(complaintId);
            return await tx.wait(1);
        };

        caseSolved(incident).then(
            () => {
                setProgressMessage("Transaction in progress...");
                console.log("Complaint closed", incident.complaintId);
                instance
                    .put(`update-fir`, incident)
                    .then((response) => {
                        console.log(response);
            ...
            (err) => {
                ...
                setIsDisabled(false);
                if (err.info.error.code == 4001) {
                    setErrorMessage(`${role} denied the ETH Transaction`);
                } else {
                    setErrorMessage("Internal Server Error. Retry again!");
                }
                openAlert(true);
    ...
    const handleWithdrawIncident = (incident) => {
        ...
        const withdrawComplaint = async ({ complaintId }) => {
            const contract = await getNFT();
            const tx = await contract.withdrawComplain(complaintId);
            return await tx.wait(1);
        };

        withdrawComplaint(incident).then(
            () => {
                setProgressMessage("Transaction in progress...");
                console.log("Complaint withdrawn", incident.complaintId);
                instance
                    .put(`update-fir`, incident)
                    .then((response) => {
                        console.log(response);
    ...
    const handleRemoveFir = (id) => {
        ...
        instance
            ...
            .then((response) => {
                console.log(response.data);
                setStatus("success");
                setErrorMessage("FIR deleted successfully");
                setTimeout(() => {
                    setIsDisabled(false);
                    openAlert();
                    setTimeout(() => {
            ...
            .catch((err) => {
                console.log("Error in deleting the FIR: " + err);
            });
    };
